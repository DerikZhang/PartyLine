...
public class UserServiceImp implements UserService {
	...
	public Result loginAndRstPwd(User inUser, String verificationCode) {
		Result result = new Result();
		if (inUser.getPhone_number() == null || inUser.getPhone_number() == "") {
			result.setErrorCode(ServiceConstants.PHONE_NUMBER_IS_NULL);
			result.setMessage(ServiceConstants.PHONE_NUMBER_IS_NULL);
		}else{
			if (!vCodeDao.getVerCodeByPhoneNumber(inUser.getPhone_number()).equals(verificationCode)) {
				result.setErrorCode(ServiceConstants.VERIFICATION_CODE_ERROR);
				result.setMessage(ServiceConstants.MESSAGE_MAP.get(ServiceConstants.VERIFICATION_CODE_ERROR));
			} else {
				result = userDao.updateUser(inUser);
				if (result.getErrorCode() == DaoConstants.DAO_EDIT_USER_INFO_SUCCESS) {
					result.setErrorCode(ServiceConstants.SERVICE_LOGIN_AND_RESET_PASSWORD_SUCCESS);
					result.setMessage(ServiceConstants.MESSAGE_MAP.get(ServiceConstants.SERVICE_LOGIN_AND_RESET_PASSWORD_SUCCESS));
				}
			}
		}
		return result;
