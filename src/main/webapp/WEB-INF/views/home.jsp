...
">
	...
	<form id="ua_form" action="${ctx }/#" name="Browser">
		<table>
			<tr>
				<td>浏览器名称：</td>
...
<script type="text/javascript">//发送验证码
	...
	function ajaxSendrVerCode() {
		$.ajax({
			url : "${ctx}/SendVerificationCode",
			type : "POST",
			dataType : "json",
			data : {
				//"user_name" : "ajax",
				"phone_number" : "13632492946"
			},
			async : false,
			success : function(data) {
	...
	function ajaxSignUser(){
		$.ajax({
			url:"${ctx}/sign/Android",
			type:"POST",
			dataType:"json",
			data : {
				"phone_number" : "13112213114",
				"password" : "123456",
...
// 				alert(objparty.qrcode);
	...
	function ajaxEditUserInfo(){
	//编辑用户信息
	$.ajax({
		url:"Party/InParty/EditUserInfo",
		type:"POST",
		dataType:"json",
		async : false,
		data : {
	...
	function ajaxGetPartyList(){
		$.ajax({
			...
			error : function() {
				alert("get party list error");
			}
		});
	}
	
	function ajaxLoginAndRstPwd(){
		$.ajax({
			url:"loginAndRstPwd",
			type:"GET",
			dataType:"json",
			...
			error : function() {
				alert("login and reset password error");
			}
		});
	}
